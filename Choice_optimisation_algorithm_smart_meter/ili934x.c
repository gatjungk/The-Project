...
void init_display_controller()
{
	...
	write_cmd_data(POWER_CONTROL_1,				 1, "\x03");
    write_cmd_data(POWER_CONTROL_2,				 1, "\x01");
...
//for testing
	/*write_cmd_data(BACKLIGHT_CONTROL_1,			 1, "\x0F"); //corresponds to 70% of greyscale data(These bits are used to set the percentage of grayscale data)
	write_cmd_data(BACKLIGHT_CONTROL_2,			 1, "\xFF"); //corresponds to 70% of greyscale data(in moving image mode)
	write_cmd_data(BACKLIGHT_CONTROL_3,			 1, "\x0F");
	write_cmd_data(BACKLIGHT_CONTROL_4,			 1, "\xFF");
	write_cmd_data(BACKLIGHT_CONTROL_5,          1, "\x37");
	write_cmd_data(WRITE_DISPLAY_BRIGHTNESS,				 1, "\x07"); //brightness registers are active, display dimming is off, backlight control is off
	write_cmd_data(WRITE_CTRL_DISPLAY, 	1, "\x02");
	write_cmd_data(WRITE_CABC_MINIMUM_BRIGHTNESS, 1, "\x0F"); //sets the minimum brightness value of the display for CABC function*/